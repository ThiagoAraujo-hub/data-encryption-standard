...
namespace SHA256.Encrypt
{
    public class TripleDESCripter
    {
        ...
        private static void Validate(byte[] key, byte[] iv)
        {
            ...
            if (key.Length != 24)
                throw new ArgumentException("Key must be 24 bytes for TripleDES encryption.");

            if (iv.Length != 8)
                throw new ArgumentException("IV must be 8 bytes for TripleDES encryption.");
        }
    }
}
